...
- `_fix_div_style_fields()` 僅對 API spec 支援的欄位修正 font-size/text-indent（bodyTitle/SubTitle/Content）
...
- 修復 `_running` 標誌：`connect()` 現在在建立 WS task 之前設定 `_running=True`（之前靜默退出）
