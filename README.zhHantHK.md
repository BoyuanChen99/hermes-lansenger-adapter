     ...
     3|     3|     3|# Hermes 藍信轉接器
     4|     4|     4|
     5|     5|     5|> 💠 藍信 網關轉接器 + 媒體與訊息工具插件，供 Hermes Agent 使用。
     6|     6|     6|
     7|     7|     7|透過 WebSocket 長連線接收即時訊息，並透過 HTTP API 發送訊息，將 Hermes Agent 連接至 藍信 — 一個企業訊息平台。
     8|     8|     8|
     9|     9|     9|本repo包含**兩個插件**：
    10|    10|    10|
    11|    11|    11|| 插件 | 類型 | 功能說明 |
    12|    12|    12||--------|------|-------------|
    13|    13|    13|| `platforms/lansenger/` | platform | 網關頻道轉接器 — 接收與發送訊息 |
    14|    14|    14|| `lansenger-tools/` | standalone (tool) | Agent 可呼叫的工具：發送檔案/圖片、撤回訊息、發送 linkCard |
    15|    15|    15|
    16|    16|    16|## 功能特色
    17|    17|    17|
    18|    18|    18|### 平台轉接器
    19|    19|    19|- **即時訊息** — 透過 WebSocket 長連線實現
    20|    20|    20|- **Markdown 支援** — 使用 `formatText` msgType
    21|    21|    21|- **i18nAppCard** — 互動式審批流程卡片
    22|    22|    22|- **主頻道自動偵測** — 首條 p2p 訊息自動設定預設發送目標
    23|    23|    23|- **定時發送** — 透過 `standalone_sender_fn` 實現排程通知
    24|    24|    24|- **使用者授權** — 透過環境變數設定允許的使用者 / 允許所有使用者
    25|    25|    25|- **零核心修改** — 純插件模式，`git diff HEAD` 保持 PRISTINE
    26|    26|    26|
    27|    27|    27|### 媒體與訊息工具插件
    28|    28|    28|- **lansenger_send_file** — 發送任何本地檔案/圖片/影片至指定使用者或群組
    29|    29|    29|- **lansenger_send_image_url** — 從 URL 發送圖片至指定使用者或群組
    30|    30|    30|- **lansenger_revoke_message** — 撤回已發送的 藍信 訊息 🗑️
    31|    31|    31|- **lansenger_send_link_card** — 發送 藍信 linkCard 卡片訊息 🔗
    32|    32|    32|- **自動媒體類型偵測** — 根據副檔名自動分類圖片/影片/文件
    33|    33|    33|- **憑證管控** — 未設定 LANSENGER_APP_ID/SECRET 時工具自動隱藏
    34|    34|    34|
    35|    35|    35|## 快速安裝
    36|    36|    36|
    37|    37|    37|### 透過 Hermes 插件管理器（推薦）
    38|    38|    38|
    39|    39|    39|```bash
    ...
    45|    45|    45|### 手動安裝
    46|    46|    46|
    47|    47|    47|將本 repo 複製至 `~/.hermes/plugins/`：
    48|    48|    48|
    49|    49|    49|```bash
    ...
    70|    70|    70|| 變數 | 說明 | 範例 |
    71|    71|    71||----------|-------------|---------|
    72|    72|    72|| `LANSENGER_APP_ID` | Bot App ID | `your-app-id` |
    73|    73|    73|| `LANSENGER_APP_SECRET` | Bot App Secret | `your-app-secret` |
    74|    74|    74|
    75|    75|    75|**憑證路徑：** 藍信桌面端 → 通訊錄 → 智能機械人 → 個人機械人 → 點擊右側 ℹ️ 圖標查看憑證（行動端不支援查看憑證）
    76|    76|    76|
    77|    77|    77|### 可選環境變數
    78|    78|    78|
    79|    79|    79|| 變數 | 說明 | 預設值 |
    80|    80|    80||----------|-------------|---------|
    81|    81|    81|| `LANSENGER_API_GATEWAY_URL` | API 網關 URL | `https://open.e.lanxin.cn/open/apigw` |
    82|    82|    82|| `LANSENGER_ALLOWED_USERS` | 允許的使用者 ID（以逗號分隔） | — |
    83|    83|    83|| `LANSENGER_ALLOW_ALL_USERS` | 允許任何使用者（僅限開發用途） | `false` |
    84|    84|    84|| `LANSENGER_HOME_CHANNEL` | 預設定時發送聊天 ID | 自動偵測 |
    85|    85|    85|
    ...
    95|    95|    95|
    96|    96|    96|這些工具讓 Agent 能夠發送檔案、圖片和影片，撤回訊息，以及發送 linkCard 卡片 — 所有工具均可由 LLM 獨立呼叫。憑證從環境變數（LANSENGER_APP_ID/SECRET）讀取，而非從 `load_gateway_config()` 讀取。
    97|    97|    97|
    98|    98|    98|| 工具 | 參數 | 說明 |
    99|    99|    99||------|-----------|-------------|
   ...
   102|   102|   102|| `lansenger_send_file` | `chat_id`, `file_path`, `caption`?, `media_type`? | 發送本地檔案/圖片/影片至使用者或群組 |
   103|   103|   103|| `lansenger_send_image_url` | `chat_id`, `image_url`, `caption`? | 從 URL 下載圖片並以原生圖片發送 |
   104|   104|   104|| `lansenger_revoke_message` | `message_ids`, `chat_type`?, `sender_id`? | 撤回已發送的 藍信 訊息（系統提示為固定內容，不可自訂） |
   105|   105|   105|| `lansenger_send_link_card` | `chat_id`, `title`, `link`, `description`?, `icon_link`?, `pc_link`?, `from_name`?, `from_icon_link`? | 發送 藍信 linkCard 卡片訊息 |
   106|   106|   106|
   107|   107|   107|**使用範例（Agent 提示）：**
   108|   108|   108|
   109|   109|   109|```
   110|   110|   110|"將 report.pdf 發送給使用者 2285568-abc123"
   111|   111|   111|"將圖表圖片分享至專案群組聊天"
   112|   112|   112|"從此 URL 下載圖片並發送給我的同事"
   113|   113|   113|"撤回我剛發送給該使用者的訊息"
   114|   114|   114|"向使用者發送 link card，標題為「專案文件」，連結為 https://..."
   115|   115|   115|```
   116|   116|   116|
   117|   117|   117|**限制：**
   118|   118|   118|- 檔案大小上限由組織的 藍信 設定決定（無固定上限）
   119|   119|   119|- 媒體說明文字使用純文字（不支援 Markdown）— 如需 Markdown 格式文字，請另行發送
   120|   120|   120|- `lansenger_send_file` 若未指定 media_type，會根據副檔名自動偵測
   121|   121|   121|- `lansenger_revoke_message`：針對員工/群組聊天類型，必須提供 `sender_id`
   122|   122|   122|
   123|   123|   123|## 架構
   124|   124|   124|
   125|   125|   125|```
   126|   126|   126|hermes plugins install → clone to ~/.hermes/plugins/hermes-lansenger-adapter/
   127|   127|   127|                          ├── plugin.yaml                     # 根manifest（kind: bundle）
   128|   128|   128|                          ├── platforms/lansenger/            # 網關轉接器
   129|   129|   129|                          │   ├── plugin.yaml                 # manifest（kind: platform）
   130|   130|   130|                          │   ├── __init__.py                  # register() → ctx.register_platform()
   131|   131|   131|                          │   └── adapter.py                   # 完整轉接器（此處無工具處理器）
   ...
   135|   135|   135|                          │   ├── schemas.py                   # LLM面向的工具描述
   136|   136|   136|                          │   └── tools.py                     # 處理器實作
   137|   137|   137|                          ├── skills/                          # Agent決策技能
   138|   138|   138|                          │   └── lansenger-messaging.md       # 工具選擇策略 + token文件
   139|   139|   139|                          ├── README.md
   140|   140|   140|                          ├── LICENSE
   141|   141|   141|                          ├── VERSION
   142|   142|   142|                          ├── after-install.md
   143|   143|   143|                          ├── pyproject.toml                   # pip入口點
   144|   144|   144|                          └── .gitignore
   145|   145|   145|```
   146|   146|   146|
   147|   147|   147|## 依賴項
   148|   148|   148|
   149|   149|   149|- `websockets` — WebSocket 客戶端，用於長連線
   150|   150|   150|- `httpx` — HTTP 客戶端，用於 API 呼叫（媒體工具亦使用）
   151|   151|   151|
   152|   152|   152|## 升級
   153|   153|   153|
   154|   154|   154|升級到最新版本：
