...
- **lansenger_revoke_message** — 撤回已發送的訊息（傳 chat_id 自動判斷群聊/私聊）
...
- **lansenger_check_in_group** — 檢查某人/機械人是否在指定群中
...
| `lansenger_revoke_message` | `message_ids`, `chat_id`? | 撤回已發送訊息（傳 chat_id 自動判斷群聊/私聊） |
...
hermes plugins install → clone to ~/.hermes/plugins/hermes-lansenger-adapter/
                          ...
                          │   ├── adapter.py                   # 核心轉接器（多重繼承 Mixin 模組）
                          │   ├── commands.py                  # 斜線命令註冊
                          │   └── _*.py / ws_*.py             # 模組化 Mixin 檔案（WS、訊息、權杖、媒體、群組、卡片、審批、i18n）
