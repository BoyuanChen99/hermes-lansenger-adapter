...
hermes plugins install → clone to ~/.hermes/plugins/hermes-lansenger-adapter/
                          ...
                          │   └── tools.py                     # 处理器实现
                          ├── skills/                          # Agent 决策技能
                          │   └── lansenger-messaging/           # skill 目录 (SKILL.md + references/)
                          ├── README.md
                          ├── LICENSE
                          ├── VERSION
                          ├── after-install.md
...
- 文件上传改用 `/v1/app/medias/create`（支持大文件，type=video/image/file/audio，ffprobe 自动提取视频宽高时长）
- 修复 `home_channel` 缺少 `platform` 字段导致 KeyError 崩溃
