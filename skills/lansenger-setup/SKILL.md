...
| API Gateway URL | `LANSENGER_API_GATEWAY_URL` | `platforms.lansenger.extra.api_gateway_url` | ✅ | API 网关地址（必填，例如 `https://your-api-gateway-url`）。 |
...
1. **App ID**（必填）
2. **App Secret**（必填）
3. **API Gateway URL**（必填，例如 `https://your-api-gateway-url`）
...
- **群 ID 来自 `source.chat_id`** — bot 上下文中自动提供，不要调用 `lansenger_query_groups` 来获取。
- **用户 ID（staff ID）来自 `source.user_id`** — bot 上下文中自动提供。
- **蓝信客户端不暴露他人的 staff ID** — 无法直接查询。对方需主动与 bot 交互后才能在 `source.user_id` 中看到。
