...
| groupAllowFrom | `LANSENGER_GROUP_ALLOW_FROM` | `platforms.lansenger.extra.group_allow_from` | — | 允许的群 ID 列表，逗号分隔（groupPolicy=allowlist 时生效） |
...
3. **允许的群列表** — 如果设 `allowlist`，需要提供允许的群 ID
...
platforms:
  lansenger:
    extra:
      ...
      group_allow_from: "<群ID1>,<群ID2>"   # 逗号分隔（groupPolicy=allowlist 时生效）
...
export LANSENGER_GROUP_ALLOW_FROM="<群ID1>,<群ID2>"
...
2. 如果 `group_policy` 是 `allowlist`，确认群 ID 在列表中
3. 检查 `require_mention` — 如果为 `true`，用户必须 @机器人名称
4. 检查 `groups.<chatId>` 下的按群覆盖设置 — 它们优先于全局设置
5. 查看日志确认消息是否被 BLOCKED：
   ```bash
