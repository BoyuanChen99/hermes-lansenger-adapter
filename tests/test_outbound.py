...
class TestRevokeMessage:
    async def test_revoke_private_chat(self, make_adapter):
        """chat_id == owner_id → chatType=bot"""
        adapter = make_adapter()
        await _ensure_token(adapter)
        adapter._http_client = AsyncMock()
        adapter._owner_id = "user-1"

        mock_response = _make_http_response({"errCode": 0})
        adapter._http_client.post = AsyncMock(return_value=mock_response)

        result = await adapter.revoke_message(["msg-1"], chat_id="user-1")
    ...
    async def test_revoke_group_chat(self, make_adapter):
        """non-owner chat_id → chatType=group"""
        adapter = make_adapter()
        await _ensure_token(adapter)
        adapter._http_client = AsyncMock()
        adapter._owner_id = "user-1"
        ...
        result = await adapter.revoke_message(["msg-2"], chat_id="group-123")

        assert result.success is True
        call_args = adapter._http_client.post.call_args
        payload = call_args.kwargs.get("json", {})
        assert payload.get("chatType") == "group"
        assert "senderId" not in payload  # not required

    async def test_revoke_no_chat_id_defaults_to_bot(self, make_adapter):
        """No chat_id → defaults to bot"""
        adapter = make_adapter()
        await _ensure_token(adapter)
        adapter._http_client = AsyncMock()
        ...
        result = await adapter.revoke_message(["msg-3"])
...
class TestSendApproveCard:
    async def test_approveCard_basic_payload(self, make_adapter):
        adapter = make_adapter()
        await _ensure_token(adapter)
        adapter._http_client = AsyncMock()
        ...
        mock_response = _make_http_response({"errCode": 0, "data": {"msgId": "msg-ap1"}})
        adapter._http_client.post = AsyncMock(return_value=mock_response)

        await adapter.send_approve_card(
            chat_id, head_title="审批", body_title="测试审批卡片",
            body_content="请确认以下操作", buttons=[{"text": "确认", "button_theme": 1}],
        )

        call_args = adapter._http_client.post.call_args
        payload = call_args.kwargs.get("json", {})
        assert payload.get("msgType") == "approveCard"
        approve_data = payload.get("msgData", {}).get("approveCard", {})
        assert approve_data.get("head", {}).get("title") == "审批"
        assert approve_data.get("body", {}).get("title") == "测试审批卡片"
        assert len(approve_data.get("buttons", [])) == 1

    async def test_approveCard_group_routes_to_group_endpoint(self, make_adapter):
        adapter = make_adapter()
        await _ensure_token(adapter)
        adapter._http_client = AsyncMock()
        ...
        mock_response = _make_http_response({"errCode": 0, "data": {"msgId": "msg-ap2"}})
        adapter._http_client.post = AsyncMock(return_value=mock_response)

        await adapter.send_approve_card(
            chat_id, head_title="审批", body_title="测试",
            buttons=[{"text": "确认"}],
        )
        ...
        assert payload["groupId"] == "group:test-group"

    async def test_approveCard_with_status(self, make_adapter):
        adapter = make_adapter()
        await _ensure_token(adapter)
        adapter._http_client = AsyncMock()
        ...
        mock_response = _make_http_response({"errCode": 0, "data": {"msgId": "msg-ap3"}})
        adapter._http_client.post = AsyncMock(return_value=mock_response)

        await adapter.send_approve_card(
            chat_id, head_title="审批", body_title="测试",
            head_status="待处理", head_status_color="#198754",
            buttons=[{"text": "确认"}],
        )
        ...
        payload = call_args.kwargs.get("json", {})
        approve_data = payload.get("msgData", {}).get("approveCard", {})
        head_status = approve_data.get("head", {}).get("headStatus", {})
        assert head_status.get("describe") == "待处理"
        assert head_status.get("colour") == "#198754"